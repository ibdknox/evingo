  ...
  for token, ok := iter.read(); ok; token, ok = iter.read() {
    if(mode == "variable") {
      ...
      case TAG:
        ...
        value, ok := iter.read()
        if !ok { fmt.Println(color.Error("Naked # on line %v"), token.line); continue; }
...
func parseAttributeLine(line *line) {
  ...
  iter := newTokenIterator(line.tokens)
  field, ok := iter.read()
  if !ok { fmt.Println(color.Error("Empty attribute on line %v"), line.line); }
  // possible cases
  //  attr
  //  attr: constant
  //  attr: var
  ...
  case op.value == "=":
    ...
    if !ok { fmt.Println(color.Error("Equality without left-hand side on line %v"), line.line); }
